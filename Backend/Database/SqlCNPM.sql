...
INSERT INTO NHANVIEN
   (MaNV, TenDangNhap, MatKhau, TenNV, NgaySinh, GioiTinh, SoDT, DiaChiNV, VaiTroKhuVucPhuTrach, TrangThaiLamViec, TrangThai)
VALUES
   ...
   ('NV02', 'sale_anh', '123456', N'Nguyễn Thị Ngọc Anh', '1998-07-22', N'Nữ', '0918776655', N'45 Nguyễn Huệ, Quận 1, TP.HCM', N'NV Bán Hàng', N'Đang làm việc', 1),
   ('NV03', 'sale_duy', '123456', N'Trần Minh Duy', '1999-11-05', N'Nam', '0933445566', N'88 Cách Mạng Tháng 8, Quận 3, TP.HCM', N'NV Bán Hàng', N'Đang làm việc', 1),
   ('NV04', 'kho_phuong', '123456', N'Lê Minh Phương', '1996-02-14', N'Nữ', '0977112233', N'234 Phạm Văn Đồng, Thủ Đức, TP.HCM', N'NV Bán Hàng', N'Đang nghỉ phép', 1),
   ('NV05', 'cskh_tuan', '123456', N'Hoàng Anh Tuấn', '2000-09-30', N'Nam', '0944556677', N'15 Nguyễn Duy Trinh, Quận 2, TP.HCM', N'NV Bán Hàng', N'Đang làm việc', 1);
GO

-- 2.2. Bảng Khách Hàng
INSERT INTO KHACHHANG
   (MaKhachHang, TenDangNhap, MatKhau, TenKhachHang, SDTKhachHang, DiaChiKhachHang, TrangThai)
VALUES
...
INSERT INTO SANPHAM
   (MaSP, MaMD, MaNhomSP, TenSP, DonViTinh, SoLuongTon, GiaBan, MoTa, HinhAnh, TrangThai)
VALUES
...
VALUES
   ...
   ('VL08', 'SP17'),
   ('VL06', 'SP17'),
   -- Bàn Tròn Đá (Đá Cẩm Thạch + Thép)
   ('VL07', 'SP29'),
   -- Ghế Nhựa (Nhựa Composite)
   ('VL02', 'SP41'),
   -- Tủ MDF
   ('VL01', 'SP42');                   -- Tủ Gỗ Sồi
GO
...
-- 5.1. Bảng Hóa Đơn
INSERT INTO HOADON
   (MaHD, MaNV, MaKhachHang, NgayLapHD, NgayGiaoHang, TrangThaiGiaoHang)
VALUES
...
INSERT INTO CHITIETHOADON
   (MaHD, MaChiTietHD, MaSP, SoLuongBan, DonGiaBan, ThanhTien, GiamGia)
VALUES
   ('HD001', 'CT01_01', 'SP01', 1, 12500000.00, 12000000.00, 500.00),
   -- Khách mua 1 bộ Sofa Góc (Giảm giá 500k)
   ('HD001', 'CT01_02', 'SP11', 1, 2100000.00, 2100000.00, 0.00),
   -- Mua kèm 1 bàn kính
   ('HD002', 'CT02_01', 'SP15', 5, 1200000.00, 6000000.00, 0.00),
   -- Khách cty mua 5 bàn NV
   ('HD002', 'CT02_02', 'SP23', 5, 18500000.00, 92500000.00, 0.00),
   -- Kèm 5 ghế xoay
...
ALTER TABLE KHACHHANG
ADD VaiTro NVARCHAR(50) NOT NULL
    CONSTRAINT DF_KHACHHANG_VaiTro DEFAULT N'Khách hàng';

-- ============================================================
-- 1) NHANVIEN: thêm cột VaiTro nếu chưa có
-- ============================================================
IF NOT EXISTS (
    SELECT 1
FROM sys.columns
WHERE object_id = OBJECT_ID('dbo.NHANVIEN') AND name = 'VaiTro'
)
BEGIN
   ALTER TABLE NHANVIEN ADD VaiTro NVARCHAR(50) NULL;
END
GO

-- Backfill: mặc định tất cả Nhân viên là "NV Bán Hàng", riêng NV01 (tài khoản admin gốc) -> "Quản trị Hệ thống"
-- Bạn có thể đổi điều kiện WHERE bên dưới nếu tài khoản Admin của bạn có mã khác NV01.
UPDATE NHANVIEN SET VaiTro = N'NV Bán Hàng' WHERE VaiTro IS NULL;
GO
UPDATE NHANVIEN SET VaiTro = N'Quản trị Hệ thống'
WHERE MaNV = 'NV01' OR VaiTroKhuVucPhuTrach LIKE N'%trị%' OR VaiTroKhuVucPhuTrach LIKE N'%Quản lý%';
GO

-- Sau khi backfill xong mới ép NOT NULL (an toàn hơn ép ngay từ đầu khi có dữ liệu cũ)
ALTER TABLE NHANVIEN ALTER COLUMN VaiTro NVARCHAR(50) NOT NULL;
GO

-- Đảm bảo NHANVIEN.Email/SecurityStamp không NULL (script cũ của bạn add NULL - giờ backfill + ép NOT NULL)
UPDATE NHANVIEN SET Email = LOWER(MaNV) + N'@chbnt.vn' WHERE Email IS NULL OR Email = '';
GO
UPDATE NHANVIEN SET SecurityStamp = CONVERT(varchar(50), NEWID()) WHERE SecurityStamp IS NULL OR SecurityStamp = '';
GO
ALTER TABLE NHANVIEN ALTER COLUMN Email VARCHAR(150) NOT NULL;
GO
ALTER TABLE NHANVIEN ALTER COLUMN SecurityStamp NVARCHAR(50) NOT NULL;
GO

-- ============================================================
-- 2) KHACHHANG: thêm cột Email, SecurityStamp, GoogleId nếu chưa có
-- ============================================================
IF NOT EXISTS (
    SELECT 1
FROM sys.columns
WHERE object_id = OBJECT_ID('dbo.KHACHHANG') AND name = 'Email'
)
BEGIN
   ALTER TABLE KHACHHANG ADD Email VARCHAR(150) NULL;
END
GO
...
IF NOT EXISTS (
    SELECT 1
FROM sys.columns
WHERE object_id = OBJECT_ID('dbo.KHACHHANG') AND name = 'SecurityStamp'
)
BEGIN
   ALTER TABLE KHACHHANG ADD SecurityStamp NVARCHAR(50) NULL;
END
GO
...
IF NOT EXISTS (
    SELECT 1
FROM sys.columns
WHERE object_id = OBJECT_ID('dbo.KHACHHANG') AND name = 'GoogleId'
)
BEGIN
   ALTER TABLE KHACHHANG ADD GoogleId VARCHAR(100) NULL;
END
GO

-- Backfill Email/SecurityStamp cho khách hàng cũ (Email tạm dựa trên Mã KH - bạn nên cập nhật lại Email thật sau)
UPDATE KHACHHANG SET Email = LOWER(MaKhachHang) + N'@placeholder.local' WHERE Email IS NULL OR Email = '';
GO
UPDATE KHACHHANG SET SecurityStamp = CONVERT(varchar(50), NEWID()) WHERE SecurityStamp IS NULL OR SecurityStamp = '';
GO
UPDATE KHACHHANG SET VaiTro = N'Khách hàng' WHERE VaiTro IS NULL OR VaiTro NOT IN (N'Khách hàng');
GO

ALTER TABLE KHACHHANG ALTER COLUMN Email VARCHAR(150) NOT NULL;
GO
ALTER TABLE KHACHHANG ALTER COLUMN SecurityStamp NVARCHAR(50) NOT NULL;
GO

-- ============================================================
-- 3) Bảng PASSWORDRESETTOKEN (Quên mật khẩu) - tạo nếu chưa có
-- ============================================================
IF NOT EXISTS (SELECT 1
FROM sys.tables
WHERE name = 'PASSWORDRESETTOKEN')
BEGIN
...
-- ============================================================
-- 4) Ràng buộc UNIQUE cho Email/TenDangNhap (bắt buộc cho Login/Google/Forgot Password)
-- ============================================================
IF NOT EXISTS (SELECT 1
FROM sys.indexes
WHERE name = 'UQ_NHANVIEN_Email' AND object_id = OBJECT_ID('dbo.NHANVIEN'))
BEGIN
   CREATE UNIQUE INDEX UQ_NHANVIEN_Email ON NHANVIEN(Email);
END
GO

IF NOT EXISTS (SELECT 1
FROM sys.indexes
WHERE name = 'UQ_KHACHHANG_Email' AND object_id = OBJECT_ID('dbo.KHACHHANG'))
BEGIN
   CREATE UNIQUE INDEX UQ_KHACHHANG_Email ON KHACHHANG(Email);
END
GO

-- Lưu ý: TenDangNhap có thể NULL (tài khoản đăng ký qua Google không có TenDangNhap),
-- SQL Server unique index tiêu chuẩn chỉ cho phép 1 giá trị NULL => dùng filtered index để cho phép nhiều NULL.
IF NOT EXISTS (SELECT 1
FROM sys.indexes
WHERE name = 'UQ_NHANVIEN_TenDangNhap' AND object_id = OBJECT_ID('dbo.NHANVIEN'))
BEGIN
   CREATE UNIQUE INDEX UQ_NHANVIEN_TenDangNhap ON NHANVIEN(TenDangNhap) WHERE TenDangNhap IS NOT NULL;
END
GO

IF NOT EXISTS (SELECT 1
FROM sys.indexes
WHERE name = 'UQ_KHACHHANG_TenDangNhap' AND object_id = OBJECT_ID('dbo.KHACHHANG'))
BEGIN
   CREATE UNIQUE INDEX UQ_KHACHHANG_TenDangNhap ON KHACHHANG(TenDangNhap) WHERE TenDangNhap IS NOT NULL;
END
GO
...
-- ============================================================
SELECT MaNV, TenDangNhap, Email, VaiTro, VaiTroKhuVucPhuTrach
FROM NHANVIEN;
SELECT MaKhachHang, TenDangNhap, Email, VaiTro
FROM KHACHHANG;
GO
